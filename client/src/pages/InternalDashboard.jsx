...
function StatusBadge({ val }) {
  const cls = {
    ...
    completed: 'metal-badge metal-badge-completed',
    defaulted: 'metal-badge metal-badge-defaulted',
    cancelled: 'metal-badge metal-badge-cancelled',
    pending: 'metal-badge metal-badge-pending',
  };
...
const monoSilver = { fontFamily: 'JetBrains Mono, monospace', color: 'rgba(175,205,235,0.9)', fontWeight: 600 };
...
export default function InternalDashboard() {
  ...
  const loanColumns = [
    { key: 'principal_amount', label: 'Amount', render: (val) => <span style={monoStyle}>{fmt(val)}</span> },
  ...
  const donorColumns = [
    { key: 'name', label: 'Name' },
    { key: 'amount', label: 'Invested', render: (val) => <span style={monoStyle}>{fmt(val)}</span> },
    { key: 'pct_of_fund', label: '% of Fund', render: (val) => <span style={monoStyle}>{fmtPct(val)}</span> },
    { key: 'projected_return_5pct', label: '5% Return', render: (val) => <span style={monoGold}>{fmt(val)}</span> },
    { key: 'projected_total_return', label: 'Total Return', render: (val) => <span style={monoSilver}>{fmt(val)}</span> },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Page header */}
      <div className="mb-6">
        <h1 className="metal-page-title">Internal Dashboard</h1>
        <p className="metal-page-subtitle">Portfolio & NIL Management</p>
      </div>

      {/* Tabs */}
      <div className="mb-8 flex gap-2 flex-wrap" style={{ borderBottom: '1px solid rgba(50,80,115,0.2)', paddingBottom: '1rem' }}>
        {TABS.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`metal-tab${activeTab === tab.id ? ' metal-tab-active' : ''}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'overview' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            ...
            <StatCard title="PIP Co Revenue (2%)" value={fmt(stats?.pip_revenue)}             subtitle="Platform fee collected" />
            <StatCard title="Donor Returns (5%)"  value={fmt(stats?.donor_returns)}           subtitle="Investor interest paid" />
            <StatCard title="NIL Collected (7%)"  value={fmt(stats?.total_nil_collected)}     subtitle="Athlete NIL funds" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <Card title="NIL by Position">
              {nilByPosition.length > 0
                ...
                : <p className="text-center py-8 text-sm italic" style={{ color: 'rgba(100,135,175,0.6)' }}>No data available</p>
              }
            </Card>
        ...
        <>
          <div className="mb-6">
            <input
              ...
              className="metal-input md:w-80"
            />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredAthletes.map((athlete) => (
              <AthleteCard key={athlete.id} athlete={athlete} nilAmount={nilDealByAthlete[athlete.id] || 0} />
            ))}
          </div>
          {filteredAthletes.length === 0 && (
            <p className="text-center py-10 text-sm italic" style={{ color: 'rgba(100,135,175,0.6)' }}>No athletes found</p>
          )}
        </>
      )}
      ...
      {activeTab === 'donors' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <StatCard title="Total Invested"         value={fmt(donorStats?.total_invested)}          subtitle="GT donor capital" />
            <StatCard title="Total Returns Earned"   value={fmt(donorStats?.total_returns_earned)}    subtitle="Cash basis (payments made)" />
            <StatCard title="Projected Total Return" value={fmt(donorStats?.projected_total_return)}  subtitle="Principal + 5% projected" />
          </div>
